...
int praseStrToData(char *str)
{
	...
	for(i=0;i<sizeof(source)/sizeof(DATA_SOURCE);i++) {
		l=getDataFromStr(str,(DATA_SOURCE*)&source[i]);
		if(l==-2)
        {
            break;
        }
	}//sizeof(source)/sizeof(DATA_SOURCE) It means how many elements that the structure has.
	#ifdef DEBUG
		debug_data_source();
	#endif
	return 1;
